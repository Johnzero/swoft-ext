...
class Filesystem
{
    ...
    public function get($path, $lock = false)
    {
        if ($this->isFile($path)) {
            return $lock ? $this->sharedGet($path) :
                (Coroutine::id() ? Coroutine::readFile($path) : file_get_contents($path));
        }
     ...
     * @return bool
     */
    public function putPhpContents($path, array $data)
    {
        $txt = "<?php \nreturn " . var_export($data, true) . ";";

        return $this->put($path, $txt, LOCK_EX);
    }
     ...
     * @return bool
     */
    public function checkCreateFile(string $filePath)
    {
        $pathParts = pathinfo($filePath);
        if (!is_dir($pathParts['dirname'])) {
            $dirPermission = $this->defaultPermissions['dir'];
        ...
        if (touch($filePath)) {
            return true;//$this->chmod($filePath, base_convert($this->defaultPermissions['file'], 8, 10));
        }
     ...
     * @return bool
     */
    public function mkdir($fullPath, $permission = null)
    {
