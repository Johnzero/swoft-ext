...
class Admin
{
    ...
    public static function getControllerName()
    {
        ...
        //     \context()->get('controllerClass');

        $controllerClass = static::getContextAttribute('__controller__');
        if (!$controllerClass) {
            [$controllerClass,$action] = class_action();
        }
        if (!$controllerClass) {
            return '';
        }
    ...
    public static function isDebug(): bool
    {
        ...
        $file = \alias('@base/config/properties/app.php');
        if (!is_file($file)) {
            return false;
        }
    ...
    private static function loadEnv()
    {
        ...
        if (env('APP_DEBUG') === null) {
            $file = '.env';
            $filePath = \alias('@base') . DS . $file;
            if (\file_exists($filePath) && \is_readable($filePath)) {
                (new Dotenv(\alias('@base'), $file))->load();
            }
        }
...
//        $idProperty = $entities[$className]['table']['id'];
        return self::$primaryKeyMap[$className] = "id";
