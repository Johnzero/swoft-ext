...
class AdminCommand
{
    ...
    public function install(Input $input, Output $output)
    {
        ...
        $output->writeln("*    <warning>@base/public</warning>");
        $output->writeln("*    <warning>@base/config</warning>");
        $output->writeln("*    <warning>@base/resource</warning>");
        $output->writeln("----------------------------------\n");

        if (!Util::isWriteable(alias('@base/public'))) {
            $output->colored('检测到目录 @base/public 没有写权限，安装失败！', 'error');
            return;
        }
        if (!Util::isWriteable(alias('@base/config'))) {
            $output->colored('检测到目录 @base/config 没有写权限，安装失败！', 'error');
            return;
        }
        if (!Util::isWriteable(alias('@base/resource'))) {
            $output->colored('检测到目录 @base/resource 没有写权限，安装失败！', 'error');
            return;
        }
