...
trait MoveAdminConfig
{
    ...
    protected function moveAdminConfig(Output $output)
    {
        ...
        $filename = '@base/config/properties/admin.php';
...
EOF;
        ...
        $app = alias('@base/config/properties/app.php');
        $content = $files->get($app);
        if (!$content) {
            $output->colored("读取配置文件 @base/config/properties/app.php 内容失败，请在安装程序结束后手动修改配置文件！");
            return;
        }

        $files->put($app, str_replace('];', $append, $content));
        $output->writeln("配置文件 <info>@base/config/properties/app.php</info> 已追加参数成功");
